...
// `CARGO_BIN_NAME` is only defined while compiling a binary target. Keep the
// fallback so the diff engine can also be used by the visual frontend through
// the library target.
const BINARY_NAME: &str = match option_env!("CARGO_BIN_NAME") {
    Some(name) => name,
    None => "difft",
};
